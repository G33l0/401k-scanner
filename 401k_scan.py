...
def print_banner():
    ...
    print(f"{CYAN}  401k SCANNER  v2.3{RESET}".center(width))
...
def run_scan(config):
    ...
    output_file = f"{config['output_base']}_{timestamp}.txt"
    print_status(f"[*] Output will be saved to: {output_file}", CYAN)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_person):
            ...
            try:
                ...
                completed += 1
                # Determine colour based on benefit found
...
# ---------- Main Menu ----------
def main():
    config = ConfigManager.load()  # load or create default
    ...
    while True:
        ...
        if choice == '1':
            ...
            config = ConfigManager.configure()  # updates and returns new config
